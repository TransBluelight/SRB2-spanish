...
static void R_DrawVisSprite(vissprite_t *vis)
{
	...
	if (vis->mobj->sprite == moviespritenum)
	{
		...
		pcolumnofs = (INT32*)texturecolumnofs[movietexturenum];
	}
	else
	{
		pwidth = patch->width;
	...
	if (vis->scalestep)
	{
		...
		for (dc_x = vis->x1; dc_x <= vis->x2; dc_x++, spryscale += scalestep)
		{
			...
			column = (column_t *)((UINT8 *)pcolumns + (pcolumnofs[texturecolumn]));
	...
	else if (vis->cut & SC_SHEAR)
	{
		...
		for (dc_x = vis->x1; dc_x <= vis->x2; dc_x++, frac += vis->xiscale, dc_texturemid -= vis->shear.tan)
		{
			...
			column = (column_t *)((UINT8 *)pcolumns + (pcolumnofs[texturecolumn]));
#else
			column = (column_t *)((UINT8 *)pcolumns + (pcolumnofs[frac>>FRACBITS]));
#endif
